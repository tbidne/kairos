...
import Kairos.Types.TZInput qualified as Types
    ...
    qs =
      GRT.firstSuccessQ
        [ GRT.embedGitError gitDataFromGitQ,
          GRT.runGitInEnvDirQ [osstr|KAIROS_HOME|] gitDataFromGitQ,
          gitDataFromEnvQ
        ]
...
--
-- - KAIROS_MODIFIED: unix time like "1746055623"
-- - KAIROS_HASH: long hash
-- - KAIROS_SHORT_HASH: short hash
--
-- We have to convert the unix time into the intended format
-- YYYY-MM-DD.
gitDataFromEnvQ :: Q (Either GitRevError (OsString, OsString, OsString))
gitDataFromEnvQ = do
  let dateVar = [osstr|KAIROS_MODIFIED|]
  d <- GRT.embedEnvError $ fmap (>>= displayUnixTime dateVar) (GRT.envValQ dateVar)
  h <- validateHash 40 <$> GRT.envValQ [osstr|KAIROS_HASH|]
  sh <- validateHash 7 <$> GRT.envValQ [osstr|KAIROS_SHORT_HASH|]
  pure $ liftA3 (,,) d h sh
  where
    ...
    validateHash' n str
      ...
      | hasInvalidChar str =
          Left $
            "Invalid char in hash: " <> T.pack (FS.OsString.decodeLenient str)
      | otherwise = Right str
...
displayUnixTime var unixTimeOsStr = do
  ...
  let posixTime = fromInteger @POSIXTime unixSeconds
      ...
      utcFormatted = Fmt.formatTime Types.locale "%Y-%m-%d" utcTime
